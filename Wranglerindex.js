...
$.getJSON("/api/db-dump", function(db_dump){
  ...
  var specDB = function($li,callback) {
    ...
    if (label === "About") {
      ...
      if (pp) {
        ...
        thePicoOut.parent.dname = getV(pp,"dname",getP(pp,"name",undefined));
      }
      thePicoOut.children = [];
      ...
      for (; i<cLen; ++i) {
        ...
        cp.dname = getV(p,"dname",getP(pp,"name",undefined));
  ...
  if (renderDemo) {
    ...
    var getP = function(p,n,d) {
      ...
      return get(db_dump.pico,[p.id,"wrangler","vars",n],d);
    }
    ...
    var walkPico =
      function(pico,dNumber,dLeft,dTop){
        ...
        var ddname = getP(pico,"name",dNumber?"Child "+dNumber:"Owner Pico");
        pico.dname = getV(pico,"dname",ddname);
